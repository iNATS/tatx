...
const initialMethods = [
  { id: 'apple', title: 'آبل باي', subtitle: 'الدفع الافتراضي', icon: 'logo-apple', color: colors.text, isDefault: true },
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <View style={styles.currentCard}>
          <Text style={[styles.currentLabel, { textAlign: textAlignStart }]}>الطريقة المختارة</Text>
          <View style={[styles.currentRow, { flexDirection: 'row-reverse' }]}>
        ...
        {methods.map((method) => (
          <View key={method.id} style={styles.methodCard}>
            <TouchableOpacity style={[styles.methodMain, { flexDirection: 'row-reverse' }]} onPress={() => setSelectedMethod(method.id)}>
            ...
            <View style={[styles.methodActions, { flexDirection: 'row-reverse' }]}>
              {!method.isDefault && (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={[styles.row, { flexDirection: 'row-reverse' }]}>
              <View style={styles.inputHalf}><TextInput style={styles.input} value={form.expiry} onChangeText={(value) => setForm((prev) => ({ ...prev, expiry: value }))} placeholder="شهر / سنة" placeholderTextColor={colors.textTertiary} /></View>
              <View style={styles.inputHalf}><TextInput style={styles.input} value={form.cvv} onChangeText={(value) => setForm((prev) => ({ ...prev, cvv: value }))} placeholder="رمز الأمان" placeholderTextColor={colors.textTertiary} keyboardType="number-pad" secureTextEntry /></View>
            </View>

            <TouchableOpacity style={[styles.saveCardRow, { flexDirection: 'row-reverse' }]} onPress={() => setSaveCard((prev) => !prev)}>
...
const styles = StyleSheet.create({
  ...
  currentCard: { backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.md, marginBottom: spacing.md, direction: 'rtl', ...shadows.sm },
  ...
  methodCard: { backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.md, marginBottom: spacing.md, direction: 'rtl', ...shadows.sm },
  ...
  methodTitle: { color: colors.text, fontFamily: fonts.semiBold, fontSize: 15, textAlign: 'right', alignSelf: 'stretch' },
  methodSubtitle: { color: colors.textSecondary, fontSize: 12, marginTop: 4, textAlign: 'right', alignSelf: 'stretch' },
  methodIcon: { width: 46, height: 46, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  methodActions: { marginTop: spacing.md, justifyContent: 'space-between', alignItems: 'center' },
  ...
  outlineMiniText: { color: colors.text, fontFamily: fonts.semiBold, fontSize: 12, textAlign: 'right' },
  defaultText: { color: colors.primary, fontFamily: fonts.semiBold, fontSize: 12, textAlign: 'right' },
  ...
  saveCardRow: { marginTop: spacing.sm, alignItems: 'center', justifyContent: 'space-between' },
  saveCardText: { color: colors.textSecondary, fontSize: 13, textAlign: 'right' },
  ...
  outlineButtonText: { color: colors.text, fontFamily: fonts.semiBold, fontSize: 15, textAlign: 'right' },
  primaryButton: { flex: 1, backgroundColor: colors.primary, borderRadius: borderRadius.full, alignItems: 'center', paddingVertical: 14 },
  primaryButtonText: { color: colors.white, fontFamily: fonts.semiBold, fontSize: 15, textAlign: 'right' },
});
