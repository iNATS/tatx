...
export default function DeliveryDashboard() {
  const availableOrders = [
    { id: '#8291', vendor: 'مطعم قصر المندي', distance: '1.2 كم', fare: '15 ر.س' },
    { id: '#8292', vendor: 'سوبر ماركت تاتكس', distance: '2.5 كم', fare: '22 ر.س' },
  ];
    ...
    <div className="flex bg-[#F8F9FA] min-h-screen" dir="rtl">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-end md:items-center gap-6 mb-12">
          ...
          <div className="bg-green-500 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-3 shadow-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <Card className="border-none shadow-xl rounded-[2.5rem] bg-white overflow-hidden ring-4 ring-primary/10">
              ...
              <CardContent className="p-8 space-y-8">
                 <div className="flex items-center gap-6 flex-row-reverse text-right">
                    ...
                    <Button size="icon" className="w-14 h-14 rounded-2xl bg-primary text-white">
                       <Navigation className="w-6 h-6" />
                    </Button>
                 </div>
                 <Button className="w-full h-16 rounded-2xl bg-green-500 hover:bg-green-600 text-white font-black text-xl gap-3 shadow-xl">
            ...
            <div className="space-y-6">
               ...
               {availableOrders.map((order) => (
                 <Card key={order.id} className="border-none shadow-sm rounded-3xl bg-white p-6">
                    <div className="flex items-center justify-between flex-row-reverse">
                       ...
                       <Button className="rounded-2xl h-12 px-8 font-black bg-black text-white">قبول ({order.fare})</Button>
                    </div>
                 </Card>
               ))}
            </div>
          ...
          <div className="space-y-8">
            ...
            <Card className="border-none shadow-sm rounded-[2rem] bg-white p-8 overflow-hidden">
               ...
               <p className="text-xs text-muted-foreground font-bold text-right">المناطق الملونة بالأحمر تشهد طلباً مرتفعاً الآن.</p>
            </Card>
          </div>
        </div>
      </main>
